...
public class MuPDFReaderView extends ReaderView {
    ...
    protected void onHit(final HitItem item) {
        this.checkMuPDFReaderViewListener();
    ...
    public void setInkColor(final int color) {
        ...
        if (null != currentView && currentView instanceof MuPDFView) {
            ((MuPDFView) currentView).setInkColor(color);
        }
    }
    ...
    public float getCurrentScale() {
        View currentView = this.getCurrentView();
        if (null != currentView && currentView instanceof MuPDFView) {
            return ((MuPDFView) currentView).getCurrentScale();
        }
        return 0;
    }
    ...
    public boolean onSingleTapUp(final MotionEvent e) {
        if (this.mMode == Mode.Viewing && !this.tapDisabled) {
            ...
            final HitItem item = pageView.passClickEvent(e.getX(), e.getY());
            this.onHit(item);
            if (item.getHit() == Hit.Nothing) {
                final LinkInfo link;
    ...
    private void checkMuPDFReaderViewListener() {
        if (this.listener == null) {
            this.listener = new MuPDFReaderViewListener() {
                ...
                @Override
                public void onHit(final HitItem item) {
                }
            };
        }
    }
